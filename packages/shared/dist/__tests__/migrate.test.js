...
function makeFakePool(options) {
    ...
    const appliedMigrations = options.appliedMigrations ?? new Set();
...
const INIT_SQL_PATH = path.resolve(_thisDir, "..", "..", "..", "..", "db", "migrations", "0001_init.sql");
// ---------------------------------------------------------------------------
// Test suite
// ---------------------------------------------------------------------------
describe("migrate.ts", () => {
    ...
    describe("AC1 — idempotency: migrations applied exactly once", () => {
        ...
        it("runs migrations in numeric filename order (lexicographic on zero-padded names)", async () => {
            ...
            fakePoolData.client.query = vi.fn(async (sql, params) => {
        ...
        it("records applied migration names in the migrations table", async () => {
            ...
            const insertCalls = fakePoolData.client.query.mock.calls.filter((args) => /INSERT INTO migrations/i.test(args[0]));
