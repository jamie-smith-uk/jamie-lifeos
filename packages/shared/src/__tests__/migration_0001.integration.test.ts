...
async function applyMigration(client: pg.PoolClient): Promise<void> {
...
describe("T-04 — 0001_init.sql integration", () => {
  ...
  describe("AC2 — conversation_context column schema", () => {
    ...
    async function getColumn(columnName: string) {
  ...
  describe("AC4 — active_confirmation JSONB nullable column", () => {
    ...
    it("inserting a row without active_confirmation stores NULL", async () => {
      ...
      const result = await client.query<{ active_confirmation: unknown }>(
      ...
      await expect(
        client.query(
          `INSERT INTO conversation_context (chat_id, role, content, active_confirmation)
           VALUES (99, 'assistant', 'please confirm', $1)`,
    ...
    it("active_confirmation can be set to NULL after being set to a JSONB value", async () => {
      ...
      await client.query(
        `INSERT INTO conversation_context (chat_id, role, content, active_confirmation)
         VALUES (77, 'assistant', 'please confirm', $1)`,
        [JSON.stringify(payload)],
      );
      await client.query(
      ...
      const result = await client.query<{ active_confirmation: unknown }>(
        `SELECT active_confirmation FROM conversation_context WHERE chat_id = 77`,
      );
      expect(result.rows[0]?.active_confirmation).toBeNull();
    });
  });
  ...
  describe("Functional smoke tests", () => {
    ...
    it("can insert and retrieve conversation_context rows by chat_id ordered by created_at DESC", async () => {
      ...
      await client.query(
        `INSERT INTO conversation_context (chat_id, role, content)
         VALUES ($1, 'assistant', 'second message')`,
        [chatId],
      );
